...
def getEvents():
    ...
    start_date = datetime.datetime(
        dt.year, dt.month, dt.day, dt.hour, 0, 00, 0, tzinfo=BR).isoformat()
    end_date = datetime.datetime(
        dt.year,  dt.month,  dt.day, 23, 30, 59, 0, tzinfo=BR).isoformat()
...
def getEventsWeek():
    ...
    firstdayofweek = datetime.datetime.strptime(
        f'{dt.year}-W{int(week_number)}-1', "%Y-W%W-%w").date()
